...
OUT="dist/ai-trainer-level3-lab-v${VERSION}"

echo "[1/6] Generating empty seed DB"
python3 scripts/build_release_db.py

echo "[2/6] Building Docker image"
docker build -t ai-trainer-level3-lab:latest .

echo "[3/6] Saving image to tar.gz"
rm -rf "$OUT"
mkdir -p "$OUT"
docker save ai-trainer-level3-lab:latest | gzip -1 > "$OUT/ai-trainer-level3-lab.tar.gz"
...
启动脚本会自动导入 ai-trainer-level3-lab.tar.gz 镜像，然后启动服务。
启动完成后访问：http://localhost:8097
EOF

echo "[6/6] Creating zip bundle"
(cd dist && rm -f "ai-trainer-level3-lab-v${VERSION}.zip" && zip -qr "ai-trainer-level3-lab-v${VERSION}.zip" "ai-trainer-level3-lab-v${VERSION}")

echo
echo "Done. Bundle dir: $OUT/"
du -sh "$OUT" "dist/ai-trainer-level3-lab-v${VERSION}.zip"
